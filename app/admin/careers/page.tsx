...
export default async function AdminCareersPage() {
  ...
  const [postings, applications] = await Promise.all([
    dbSelectAuth<JobPosting>('job_postings', token, { order: 'created_at.desc' }),
    dbSelectAuth<JobApplication>('job_applications', token, { order: 'created_at.desc' }),
  ])
    ...
    <div className="space-y-12">
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="font-serif text-3xl text-chm-black">Job Postings</h1>
            <p className="text-sm text-gray-400 mt-1">{postings.length} posting{postings.length !== 1 ? 's' : ''}</p>
          </div>
              ...
              <thead>
                <tr className="border-b-2 border-gray-200 bg-gray-50">
                  {['Title', 'Category', 'Status', ''].map(h => (
                    <th key={h} className="text-left py-3 px-4 text-xs uppercase tracking-widest text-gray-500 font-semibold">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {postings.map(p => (
                  <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 text-gray-500 text-xs">{p.service_category ?? '—'}</td>
                    <td className="py-3 px-4">
                      <span className={`text-xs px-2 py-0.5 ${p.active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
                        {p.active ? 'Active' : 'Hidden'}
                      </span>
                    </td>
                    <td className="py-3 px-4">
                      <Link href={`/admin/careers/${p.id}`}
      ...
      <div>
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="font-serif text-2xl text-chm-black">Applications</h2>
