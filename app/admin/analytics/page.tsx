...
export default function AdminAnalyticsPage() {
  ...
  const handleDelete = useCallback(async (table: 'offer_letters' | 'staff_module_progress', id: string, name: string) => {
    if (!confirm(`Are you sure you want to delete this ${table === 'offer_letters' ? 'offer' : 'progress'} record for ${name}? This cannot be undone.`)) {
      return
    }
  ...
  const { progress, modules, offerLetters } = data
    ...
    <div>
                ...
                {offerLetters
                  ...
                  .map((offer) => (
                    <tr key={offer.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <button
                          onClick={() => handleDelete('offer_letters', offer.id, offer.offer_letter_applicants?.full_name || 'Offer')}
      ...
      <div>
        ...
        <div>
          ...
          <StaffRecordsTable records={merged} onDelete={handleDelete} />
        </div>
      </div>
    </div>
  )
}sName="py-3 px-4 text-gray-600">{p.completed}</td>
                    <td className="py-3 px-4 font-semibold text-chm-red">{p.passRate}%</td>
                  </tr>
                ))}
              </tbody>
